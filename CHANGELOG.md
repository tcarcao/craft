...
- **`craft fmt`.** The formatter is now a CLI command, not only an LSP request. `craft fmt <files...>` formats in place; `craft fmt --check <files...>` writes nothing, lists every file that is not already formatted, and exits non-zero, which is the CI gate. Arguments are paths or globs (including `**`) resolved exactly as `craft validate` resolves them; directories are not walked. A file the parser cannot fully place is never rewritten, and is reported as skipped with the diagnostic that blocked it rather than being passed over in silence.
  - Read the known limitation under `### Notes` before pointing `--check` at a whole repository. `context_map` and `glossary` blocks are not yet safe to format.
...
### Fixed
- **`textDocument/formatting` had five pre-existing defects in `use_case` bodies, all found while building this release.** Each silently mutated a valid document, and none had test coverage before now. Formatting of `use_case` bodies is now verified byte-identical, reparse-clean, idempotent, and model-preserving across all four action kinds and all trigger forms. It is only `use_case` bodies: see the known limitation under `### Notes` for the blocks that are still unsafe.
  ...
  - Format Document corrupted qualified references **in `use_case` bodies**, rendering `when re/billing listens vas.VasApplied` as `when re / billing listens vas.VasApplied`, which no longer parsed. The same corruption still occurs in `context_map` and `glossary` blocks, which are rendered by a different code path; see the known limitation under `### Notes`.
...
- **Known limitation: `context_map` and `glossary` blocks are not yet safe to format.** The verified formatting work in this release covers `use_case` bodies. Every other block still goes through a generic renderer that space-joins tokens, which splits a qualified `billing/Invoice` into `billing / Invoice` and loses the line breaks between relations, so the output no longer parses. These are precisely the blocks where qualified refs are the native spelling, so they are the ones it hits hardest. Pre-existing, not introduced here, and verified byte-identical against the previous release. Until it is fixed, do not run `craft fmt` or Format Document over files containing those blocks. A follow-up will extend the source-faithful rendering used for `use_case` bodies to the rest.
