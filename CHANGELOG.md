...
**Why a minor version.** `pkg/craft`'s exported API is unchanged: the formatter lives in `internal/lsp`, nothing exported was removed or altered, and the module path stays `github.com/tcarcao/craft/v2`. Under Go module semantics that is a minor. Read the first entry before upgrading anyway, because **output changes for every existing file**: the default indent moves from 2 spaces to 4, so `craft fmt` will rewrite files it previously left alone and `craft fmt --check` will report previously-clean files as unformatted until each workspace either accepts the reflow or adds a `.craftfmt` containing `indent = 2`.

### Changed
- **The default indent is 4 spaces, was 2.** Every `.craft` file reformats on the next `craft fmt`. A workspace that wants the old output adds a `.craftfmt` with `indent = 2` at its root; nothing else is required, and the setting is picked up by both the CLI and the language server. If you use the VS Code extension, set `editor.tabSize` under `[craft]` to the same value, because the extension declares 4 to the editor and that setting governs typing rather than formatting.
  - The repo's own 99-file corpus moved from 50 canonical files to 99 in this release, which is the same reflow every downstream workspace will see once.

### Added
- **`.craftfmt`, a per-workspace formatter configuration.** A TOML file resolved from the formatted file's directory upward to the nearest ancestor, so a single `craft fmt` invocation spanning two workspaces honours each one's own settings. Keys, with their defaults: `indent = 4` and `continuation_indent = 4` (integers, 0 to 16), and an `[align]` table carrying `trailing_comment = "block"`, `op_annotation = "block"`, `outlier_ratio = 2.5` and `outlier_min = 40`. An unknown key is an error rather than a silent no-op, because a typo that quietly does nothing is worse than one that fails; a malformed file fails only its own workspace and names the file, the key, and the legal values.
  - The full key reference lives in the CLI reference alongside the flags, not only in the architecture decision record.
- **`--indent`, `--continuation-indent` and `--align-trailing-comment` on `craft fmt`**, so CI can override a workspace's file without editing it. Flags merge over `.craftfmt` **per field**: passing `--indent 4` does not discard a `trailing_comment` the config file set, and only a flag the user actually typed overrides anything.
- **Trailing `//` comments are column-aligned**, using the same run machinery that already aligned `[POST /v1/...]` operation annotations. This restores a shape the formatter previously destroyed: a hand-aligned comment column, such as the rollup annotations in a service registry, collapsed to a single space on the first `craft fmt`.
  - The scope is configurable through `align.trailing_comment`, ordered least-aligning to most: `off`, `strict` (any line without a comment ends the run, as gofmt and hclwrite do), `block` (a blank line, `{`, `}`, or comment-only line ends it), `file` (only a blank line), and `decl` (nothing ends it, so one column spans a whole declaration).
  - `block` is the default because it bounds churn. A single new long name re-indents its own block rather than every commented line in the file, which is the difference between a 4-line and a 111-line diff on a real registry. A workspace that prefers one uniform column across a whole `domains { }` listing sets `decl`.
  - `align.op_annotation` carries the same vocabulary and defaults to `block`, which preserves the annotation behaviour documented in 2.17.0 exactly.
- **A width-outlier guard**, following gofmt's `exprList` rule: a body more than `outlier_ratio` times the geometric mean of its run is excluded from setting the column, but only once that mean exceeds `outlier_min`. One 300-character line no longer drags forty 50-character lines out to match it. The guard is deliberately weak and does not fire on ordinary variation; bounding ordinary churn is the scope setting's job. Set `outlier_ratio = 0` to disable it.

### Fixed
- **A wrapped list continuation now hangs instead of flattening.** The formatter preserved the author's line break inside a wrapped `contexts:` value but reset the indent to block depth, so the last item of the list sat at the same column as its key and was visually indistinguishable from a sibling statement in the enclosing block. Continuations now indent one `continuation_indent` past the block indent.
  - The column is derived from nesting depth, never from the measured width of the emitted line above, which is what keeps formatting a fixed point when alignment shifts a neighbouring line.
  - This follows the direction every formatter that revisited the question since 2016 has taken: rustfmt defaults to block indent and rejects visual indent on the stable channel, scalafmt flipped its default in v1.6, and Google's Java style names the cost as corrupted version history and worse merge conflicts. clang-format is the remaining holdout.
- **`craft fmt` no longer refuses a file whose errors it did not cause.** The reparse guard checked only the formatted output's diagnostics, so any file carrying a pre-existing semantic error and not already byte-canonical was skipped with `formatting would have broken the file` when formatting had broken nothing, and could never be normalised. The guard now compares against the input's own diagnostics and bails only on an error formatting actually introduced.
  - It had been invisible because a file that was already canonical short-circuited before ever reaching the guard. Changing the default indent is what walked it into the light.
- **The language server says why it declined to format a file.** It previously returned no edits and no explanation, while `craft fmt` reported `skipped, not formatted: <reason>` for the same file, so the same condition was silent in the editor and explained on the command line. The reason now arrives as a `window/showMessage`, carrying the same text and diagnostic code the CLI prints. It is deduplicated per document against the full diagnostic, so a repeated save does not repeat the message while a genuinely different reason still surfaces.
- **The language server honours `.craftfmt`**, so a file formats identically on save and from the command line. It deliberately ignores the editor's `tabSize` and `insertSpaces`: VS Code's `editor.detectIndentation` infers those per file from that file's existing content, so honouring them would make an existing 2-space file format at 2 in the editor and 4 from the CLI, permanently and with no error. The extension declares craft's width to the editor instead of the server asking for it.
...
### Fixed
- **A `when` block outside any `use_case` is no longer dropped in silence.** The parser discarded the block and everything in it, with no scenario, actions, or participants reaching the model, and reported one `craft/syntax/not-yet-implemented` **warning** pointing at the four characters of `when`. `craft validate` exited 0, so every consumer built on the model, and every CI gate built on the exit code, agreed the lines had never been written. Measured on one real file, a stray `}` had been costing one scenario and four actions for as long as the typo had been there, across several review passes.
  - **The code is split.** `craft/syntax/not-yet-implemented` keeps its codebook meaning, "the grammar recognises this, parser v2 does not implement it yet": the tool is behind, the file is fine, and warning is right. It now covers only that case, which today is a trailing `[operation]` annotation on a line whose phrase contains braces. A token that cannot be placed at all, and whose region is therefore discarded, gets the new `craft/syntax/skipped-construct` at **error**. This aligns the behaviour with `docs/DIAGNOSTICS.md`, which puts every `craft/syntax/*` entry at `error` and reserves `warning` for `craft/lint/*` style findings, rather than changing policy.
  - **The extent of the loss is reported.** The range used to cover the offending token while recovery ate everything up to the next top-level keyword, which can be dozens of lines. It now spans the whole discarded region, and the message says so: `unexpected "when" at top level: lines 6-8 were skipped and are not part of the model`.
  - **The cause is named, not the symptom.** Nobody writes a top-level `when` on purpose; what happens is one extra `}` that closed a `use_case` early, four lines above where the diagnostic pointed. When a `use_case` closed cleanly and nothing else was parsed since, the message now blames that brace: `"Settle a seller invoice" (opened at line 1) was already closed at line 4, so that `}` may be extra`. A `use_case` that bailed to error recovery records no close, so no brace is guessed at.
  - **`--strict` is no longer the only lever.** Promoting all warnings meant adopting every pre-existing lint finding as a build failure to gate on dropped content. Content loss is now an error on its own, so `craft validate` fails on it with no flags, and linting stays advisory.
  - Not affected: `craft fmt` already round-tripped the orphaned block byte for byte, and the formatter's bail-out already covered this case by code; it is now covered by severity as well.

## [2.17.1] — 2026-08-14

**Why a patch version.** `pkg/craft`'s exported API is unchanged, so nothing stops compiling. Two bugs are fixed, both found by running `craft validate` over a 193-file real-world architecture-documentation corpus (~543 use cases), where between them they accounted for essentially every reported finding. Behaviour reachable through the API does move: documents that previously reported errors now validate, and the diagnostic slice is now stably ordered and stably attributed. If you pin golden diagnostic output, expect it to move once, to a correct value.

### Fixed
- **A domain-qualified use-case participant now resolves.** `Subscriptions asks re/billing for a fresh charge attempt` was reported as `craft/sema/unresolved-reference`, as was every other `<domain>/<name>` reference in a `use_case` body: any action's subject, the `asks` target, the `returns` target, and the `when ... listens` trigger context. The qualified form has always been part of the grammar and has always been documented (see `docs/page/language/use-cases.md`), and `context_map` endpoints have always accepted it, so this was one half of the language implementing what the grammar says while the other half did not.
  - Cause: `resolveUseCaseRef` looked the participant up by bare name only, against actors, then domains, then bounded contexts, then services. A name containing a `/` matched none of those and fell through to the unresolved branch. It now splits on the first `/`, resolves the domain, and confirms that domain declares the bounded context, mirroring `resolveBCRef`. A qualifier naming an unknown domain, or a domain that does not declare that context, is still unresolved.
  - This matters beyond style: a bounded context whose name is declared in two domains has no unambiguous bare spelling, so rejecting the qualified form made those references unwritable.
  - Effect on the corpus: 940 findings dropped to 98. 842 of the 940 were this one bug.
- **Diagnostics are deterministic, in content as well as in order.** Two runs over identical input returned the same findings in a different order, and 16 of the corpus's 98 findings named a *different file and line* on each run: same rule, same event, different blame. The finding count was identical every time, which is what let this go unnoticed.
  - Cause: every lint rule walked the workspace with `for uri, tree := range perFileTrees` while accumulating into a map keyed by something coarser than the file, such as the event name. The first file to reach a given key is the one recorded and every later file is suppressed as a duplicate, so Go's map seed decided which file got blamed. A new `sortedTreeURIs` helper is used at all seven walk sites, which makes "the first file wins" a statement about the corpus rather than about the map seed.
  - `MergeWorkspaceSymbols` had the same defect one layer lower and was reachable from both the CLI and the LSP: it merged per-file symbol tables by ranging over a map with last-write-wins, so a name declared in two files had a randomly chosen winner. That winner is what downstream consumers report as the declaration, which decided the file and line of an `unused-actor` finding, the owning domain a bare bounded-context reference resolved to, and the position go-to-definition jumped to. Files are now merged in ascending URI order and the first declaration wins, for every symbol kind, matching the rule the duplicate-service branch already documented.
  - `sortDiags` gained a `Message` tiebreak, which is what makes its comparator a total order. Without it, two diagnostics sharing a position and a code compared equal and `SliceStable` faithfully preserved whatever order the upstream map iteration produced.
  - `ParseFiles` now sorts the whole slice once before returning. Each block was sorted on its own, which ordered diagnostics within a block but said nothing about the concatenation of blocks.
  - Verification: `craft validate` over the repository's own `examples/` produced 8 distinct outputs from 8 runs before this change and 8 byte-identical outputs after, with the finding count unchanged at 218 throughout.

### Notes
- **Any file:line attribution captured from a `craft validate` run before this release may name the wrong file**, for the dedup-keyed rules specifically (`craft/lint/dead-event`, `craft/lint/unused-actor`, `craft/lint/event-not-past-tense`). The finding itself was real; the location may not have been. Anything downstream that recorded those locations should be re-derived from a current run rather than trusted.
- Neither bug had test coverage, because both are workspace-scale behaviours: the resolution bug needs a bounded-context name declared in two domains, and the attribution bug is invisible unless you run the same multi-file workspace twice and compare. Sixteen regression tests were added, including the parse-twice-and-compare shape that is the only one that could have caught the second bug.
...
- **The alignment pass no longer guesses where comment text ends.** It used to pattern-match a line for `//`, `/*`, `*/` and a leading `*` to work out which part was comment, while the walker that produced the line knew every comment token's exact extent and discarded it. The walker now hands those ends down. A comment closing on a line that starts with a word, as in `see // here */ Billing asks Gateway to charge  [POST /pay]`, lost its alignment entirely while the same comment spelled with a leading `*` aligned; both now align. The special cases the guessing needed, including an exception for the `//` in a URL, are gone rather than extended: 57 lines of heuristic deleted.
- **An empty document stays empty.** It used to format to a single newline while a whitespace-only document formatted to nothing, so formatting alternated between the two forever. Formatting is now idempotent for every input, including whitespace-only files.
