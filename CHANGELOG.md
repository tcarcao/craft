...
### Fixed
- **`textDocument/formatting` had nine pre-existing defects, all found while building this release.** Each silently mutated a valid document, and none had test coverage before now. Formatting is now verified byte-identical, reparse-clean, idempotent, model-preserving, and comment-preserving over **every** `.craft` file in the repository, not just the hand-written fixtures. That whole-repository guard is the thing that would have caught this class from the start, and it is what makes `craft fmt` safe to point at a tree.
