...
- **The parser asserts text equality rather than width equality.** `checkTreeWidth` compared the sum of token widths against the file length. Because a token's length was derived from its own text, a wrong text produced a length wrong by the same amount, so that check could not detect this class of defect at all. `checkTreeText` replaces it and compares the reconstructed text. It panics under test so a regression cannot pass CI, and returns a diagnostic in production, because `syntax.Parse` is reached from `pkg/craft` and a library must not panic in a consumer's process.
...
- **An annotation on the line where a block comment closes now aligns with its siblings.** The walker treated every line after a token's first as interior to that token, which over-claimed the last line the token shares with whatever follows it.
